...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, neighbor_index_space, cent_obs_space, act_space):
        ...
        neighbor_index_space_shape = get_shape_from_obs_space(neighbor_index_space)
        ...
        self.adj_matrix = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, num_agents, num_agents), dtype=np.float32)
        self.neighbor_index = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, *neighbor_index_space_shape), dtype=np.int32)

        self.step = 0

    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, adj_matrix, neighbor_index, bad_masks=None, active_masks=None, available_actions=None):
        """
        ...
        self.adj_matrix[self.step + 1] = adj_matrix.copy()
        self.neighbor_index[self.step + 1] = neighbor_index.copy()
        ...
        self.step = (self.step + 1) % (self.episode_length + 1)

    def clip(self):
        self.share_obs = self.share_obs[:self.step + 1]
        self.obs = self.obs[:self.step + 1]
        self.rnn_states = self.rnn_states[:self.step + 1]
        self.rnn_states_critic = self.rnn_states_critic[:self.step + 1]
        self.actions = self.actions[:self.step]
        self.action_log_probs = self.action_log_probs[:self.step]
        self.value_preds = self.value_preds[:self.step + 1]
        self.rewards = self.rewards[:self.step]
        self.masks = self.masks[:self.step + 1]
        self.bad_masks = self.bad_masks[:self.step + 1]
        self.active_masks = self.active_masks[:self.step + 1]
        if self.available_actions is not None:
            self.available_actions = self.available_actions[:self.step + 1]
        self.returns = self.returns[:self.step + 1]
    ...
    def after_update(self):
        ...
        self.adj_matrix[0] = self.adj_matrix[self.step].copy()
        self.neighbor_index[0] = self.neighbor_index[self.step].copy()
    ...
    def chooseafter_update(self):
        ...
        self.adj_matrix[0] = self.adj_matrix[-1].copy()
        self.neighbor_index[0] = self.neighbor_index[-1].copy()
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        neighbor_index = self.neighbor_index[:-1].reshape(-1, *self.neighbor_index.shape[3:])
        ...
        adj_matrix = self.adj_matrix[:-1].reshape(-1, num_agents , num_agents)
        ...
        for indices in sampler:
            ...
            neighbor_index_batch = neighbor_index[indices]
            ...
            adj_matrix_batch = adj_matrix[indices]
            ...
            yield share_obs_batch, obs_batch, neighbor_index_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                  value_preds_batch, return_batch, masks_batch, adj_matrix_batch, active_masks_batch, old_action_log_probs_batch,\
