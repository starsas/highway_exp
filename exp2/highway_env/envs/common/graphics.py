...
class EnvViewer(object):
    ...
    # def _draw_busyness_map(self):
    #     """绘制繁忙度图颜色块到 sim_surface 上"""
    #     # 创建一个 Matplotlib figure 和 canvas
    #     fig, ax = plt.subplots(figsize=(self.env.config["screen_width"] / 100,
    #                                     self.env.config["screen_height"] / 100), dpi=100)
    #     busyness_map(self.env, ax=ax)  # 使用你已经定义的 busyness_map 函数

    #     # 将 Matplotlib 图像绘制到 Pygame 表面
    #     canvas = FigureCanvas(fig)
    #     canvas.draw()
    #     raw_data = canvas.buffer_rgba()
    #     size = canvas.get_width_height()

    #     # 将 memoryview 转换为 bytes 并创建 Pygame surface
    #     busy_surface = pygame.image.fromstring(raw_data.tobytes(), size, "RGBA")

    #     # 使用平滑缩放以防止失真
    #     scaled_surface = pygame.transform.smoothscale(busy_surface, (self.env.config["screen_width"], self.env.config["screen_height"]))

    #     # 叠加到主渲染表面
    #     self.sim_surface.blit(scaled_surface, (0, 0))
