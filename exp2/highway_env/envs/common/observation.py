...
class OccupancyGridObservation(ObservationType):
    ...
    def __init__(self,
        ...
        grid_shape = np.asarray(np.floor((self.grid_size[:, 1] - self.grid_size[:, 0]) / grid_step), dtype=np.int32)
...
class MultiAgentObservation(ObservationType):
    ...
    def observe(self) -> tuple:
        ...
        return tuple(obs_type.observe()[0] for obs_type in self.agents_observation_types), \
               tuple(obs_type.observe()[1] for obs_type in self.agents_observation_types),\
                tuple(obs_type.observe()[2] for obs_type in self.agents_observation_types)
