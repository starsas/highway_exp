...
class RoadNetwork(object):
    ...
    def side_lanes(self, lane_index: LaneIndex) -> List[LaneIndex]:
        ...
        # if _id < len(self.graph[_from][_to]) - 1 and _id != 2:
        #     lanes.append((_from, _to, _id + 1))
        # if _id == 2 and _from == "c" and _to == "d":
        #     lanes.append((_from, _to, 3))
        # if _id == 3 and _from == "c" and _to == "d":
        #     lanes.append((_from, _to, 2))
        if _id > 0 and _id != 3:
            lanes.append((_from, _to, _id - 1))
        if _id < len(self.graph[_from][_to]) - 1 and _id != 2 and _from != "d" :
            lanes.append((_from, _to, _id + 1))
        return lanes
...
class Road(object):
    ...
    def surrounding_vehicles(self, vehicle: 'kinematics.Vehicle', env_copy = False, lane_index: LaneIndex = None) \
        ...
        for v in self.vehicles:
            if (v.id != vehicle.id) and not isinstance(v, Landmark):
                # 考虑三条主路和匝道
                # print(lane_index[2])
                if lane_index[2] in [0, 1, 2, 3]:
                    current_segment = set(lane_index[0:2])
                    vehicle_segment = set(v.lane_index[0:2])
                    # 只需要判断当前车辆和目标车辆的路线起点终点是否有交集，是否在同一根车道即可
                    if v.position is not None and len(v.position) == 2:
                        s_v, lat_v = v.position
                    else:
                        continue # 如果 position 无效，跳过当前车辆
            else:
                continue # 如果车道或段不匹配，跳过当前车辆
                # # 考虑匝道,好像可以合并
                # else:
                #     if lane_index == ("a", "c", 3) and (
                #             v.lane_index == ("a", "c", 3) or v.lane_index == ("c", "d", 3)):
                #         s_v, lat_v = v.position
                #     elif lane_index == ("c", "d", 3) and (
                #             v.lane_index == ("a", "c", 3) or v.lane_index == ("c", "d", 3)):
                #         s_v, lat_v = v.position
                #     else:
                #         continue

                # 找当前车辆紧挨着的前方和后方的车，返回车辆的索引
        ...
        if env_copy:
            if v_front is not None:
                v_front = vehicle.road.vehicles[v_front.id]
            if v_rear is not None:
                v_rear = vehicle.road.vehicles[v_rear.id]
