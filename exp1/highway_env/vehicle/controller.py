...
class ControlledVehicle(Vehicle):
    ...
    def act(self, action: Union[dict, str] = None) -> None:
        ...
        if action == "FASTER":
            ...
            # 因为匝道与主干道字段一致，但不连接，需特殊判断
            if self.target_lane_index in [("a", "b", 1), ("b", "c", 1)]:
                target_lane_index = _from, _to, 1
            else:
            ...
            _from, _to, _id = self.target_lane_index
            # 因为匝道与主干道字段一致，但不连接，需特殊判断
            if self.target_lane_index in [("a", "b", 2), ("b", "c", 2)]:
                target_lane_index = _from, _to, 2
            else:
                target_lane_index = _from, _to, np.clip(_id - 1, 0, len(self.road.network.graph[_from][_to]) - 1)
            if self.road.network.get_lane(target_lane_index).is_reachable_from(self.position):
                self.target_lane_index = target_lane_index
    ...
    def follow_road(self) -> None:
        ...
        # if self.road.network.get_lane(self.lane_index).after_end(self.position):
...
class MDPVehicle(ControlledVehicle):
    ...
    def act(self, action: Union[dict, str] = None) -> None:
        ...
        """
        if action == "FASTER":
            self.speed_index = self.speed_to_index(self.target_speed) + 1
        elif action == "SLOWER":
            self.speed_index = self.speed_to_index(self.target_speed) - 1
        elif action == "BRAKE":
            self.speed_index = 0
    ...
    def index_to_speed(self, index: int) -> float:
        ...
        """
        if (self.SPEED_COUNT>1) and (index>0):
    ...
    def speed_to_index(self, speed: float) -> int:
        ...
        return np.int32(np.clip(np.round(x * (self.SPEED_COUNT - 1)), 0, self.SPEED_COUNT - 1))
    ...
    def speed_to_index_default(cls, speed: float) -> int:
        ...
        return np.int32(np.clip(np.round(x * (cls.SPEED_COUNT - 1)), 0, cls.SPEED_COUNT - 1))
